...
enum class SessionManager(
    ...
    NONE("None", null, null),
    TMUX("tmux",
        { name -> "if ! command -v tmux >/dev/null 2>&1; then echo 'Haven: tmux not found. Install it (e.g. apt install tmux) or change session manager in connection settings.'; else tmux new-session -A -s $name \\; set -gq allow-passthrough on \\; set -gq mouse on; fi" },
        "tmux ls -F '#{session_name}' 2>/dev/null",
        { name -> "tmux kill-session -t $name" },
        { old, new -> "tmux rename-session -t $old $new" },
    ),
    ZELLIJ("zellij",
        { name -> "if ! command -v zellij >/dev/null 2>&1; then echo 'Haven: zellij not found. See https://zellij.dev/documentation/installation or change session manager in connection settings.'; else zellij attach $name --create; fi" },
    ...
    SCREEN("screen",
        { name -> "if ! command -v screen >/dev/null 2>&1; then echo 'Haven: screen not found. Install it (e.g. apt install screen) or change session manager in connection settings.'; else screen -dRR $name; fi" },
        "screen -ls 2>/dev/null",
        { name -> "screen -S $name -X quit" },
        { old, new -> "screen -S $old -X sessionname $new" },
    ),
    BYOBU("byobu",
        { name -> "if ! command -v byobu >/dev/null 2>&1; then echo 'Haven: byobu not found. Install it (e.g. apt install byobu) or change session manager in connection settings.'; else byobu new-session -A -s $name \\; set -gq mouse on; fi" },
